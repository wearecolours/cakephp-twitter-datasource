	...
	public function read(Model $model, $queryData = array(), $recursive = null) {
		...
		switch (get_class($model)) {
			...
			case 'TweetApi': // TweeterApi
				...
				$oauthSignature = $this->_generateOauthSignature($url, $method, $requestParams, $requestBody, $consumerSecret, $oauthTokenSecret);
				$data = $this->_curll($url, $method, $requestParams, $consumerKey, $oauthSignature, $oauthToken);
				break;
			case 'Tweet':
 ...
 */
	 ...
	 *
	 * @param apiEndpoint {string} - API endpoint e.g. users/show
	 * @param method {string} - http method GET or POST (it has to be UPPERCASE)
	 * @param requestParams {array} - associative array of parameters sent with the request. e.g ['screen_name' => 'madonna']
	 ...
	 *
	 * @return response {json} - http api response
	 */
	protected function _curll($apiEndpoint, $method="GET", $requestParams, $customer_key, $oauth_signature, $oauth_token) {
		if (isset($requestParams) AND $method == "GET") {
	 ...
	 */
	protected function _generateOauthSignature($baseUrl, $method = "GET", $requestParams, $requestBody, $consumerSecret, $oauthTokenSecret) {
